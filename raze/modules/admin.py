...
/set UI customizer, /all mention, group management, /start dashboard
ENHANCED: Better /start with inline keyboards
"""
import asyncio
import random
import logging
...
logger = logging.getLogger("RazeChan.Admin")
...
async def set_cmd(client, msg: Message):
    """Set custom UI banner for commands"""
    parts = msg.command
...
async def cancel_set(client, msg: Message):
    """Cancel set mode"""
...
async def capture_banner(client, msg: Message):
    """Capture banner photo"""
    ...
    try:
        ...
        text = (
            ...
            f"Caption: `{caption[:100]}...`" if len(caption) > 100 
            else f"✅ **/{cmd_name}** ka banner save ho gaya!\n\nCaption: `{caption}`"
        )
        await msg.reply(text)
        logger.info(f"✅ Banner saved for /{cmd_name}")
    except Exception as e:
        logger.error(f"Error saving banner: {e}")
        await msg.reply(f"❌ Error saving banner: `{e}`")

async def get_ui(command: str) -> dict | None:
    """Get UI config for command"""
    try:
        return await ui_col.find_one({"command": command})
    except Exception as e:
        logger.error(f"Error getting UI: {e}")
        return None

# ── /start Dashboard - ENHANCED WITH BUTTONS ─────────────────────────────
@app.on_message(filters.command("start"))
async def start_cmd(client, msg: Message):
    """Start command with inline keyboard dashboard"""
    try:
        user = msg.from_user
        name = user.first_name or "Bestie"
        ...
        # Main dashboard buttons - organized in groups
        btns = InlineKeyboardMarkup([
            # Row 1: Core features
            [InlineKeyboardButton("💬 Chat", callback_data="info:chat"),
            ...
            [InlineKeyboardButton("💰 Economy", callback_data="info:economy"),
             InlineKeyboardButton("❓ Help", callback_data="help_index")],
        ...
        logger.info(f"✅ /start shown to {user.first_name} ({user.id})")
    except Exception as e:
        logger.error(f"Error in start_cmd: {e}", exc_info=True)
        await msg.reply("oops kuch glitch! 😅 refresh karo~")
...
async def help_cmd(client, msg: Message):
    ...
    try:
        ...
        text = (
            ...
            "  `/ttt` — Tic-Tac-Toe\n"
            "  `/minesweeper` — Mine clear karo\n"
            "  `/rps` — Rock Paper Scissors\n\n"
            "**👥 Group:**\n"
            "  `/all` — sabko mention karo\n\n"
            "_Mention karo, reply karo, ya naam lo — main hamesha sunti hoon~_ 👂"
        )
...
async def mention_all(client, msg: Message):
    ...
    try:
        ...
        logger.info(f"✅ /all mentioned {count} users in {chat.title}")
    except Exception as e:
        logger.error(f"Error in mention_all: {e}")
        await msg.reply("oops error listing members 😅")
...
async def games_panel_cb(client, cb: CallbackQuery):
    """Games panel"""
    try:
        await cb.answer()
        btns = InlineKeyboardMarkup([
        ...
        logger.error(f"Error in games_panel_cb: {e}")
        await cb.answer("Error!", show_alert=True)

@app.on_callback_query(filters.regex("^aura_panel$"))
async def aura_panel_cb(client, cb: CallbackQuery):
    """Aura panel"""
    try:
        await cb.answer()
        from raze.modules.aura import get_aura, get_realm
        ...
        logger.error(f"Error in aura_panel_cb: {e}")
        await cb.answer("Error!", show_alert=True)

@app.on_callback_query(filters.regex("^realms_info$"))
async def realms_info_cb(client, cb: CallbackQuery):
    """Realms info"""
    try:
        await cb.answer()
        ...
        logger.error(f"Error in realms_info_cb: {e}")
        await cb.answer("Error!", show_alert=True)

@app.on_callback_query(filters.regex("^info:"))
async def info_panel(client, cb: CallbackQuery):
    """Info panels"""
    try:
        await cb.answer()
        section = cb.data.split(":")[1]
        info_map = {
            "chat": "💬 **AI Chat**\n\nMention me, reply me, ya naam lo → instant reply! Gen-Z Hinglish style~ 🥺",
        ...
        logger.error(f"Error in info_panel: {e}")
        await cb.answer("Error!", show_alert=True)

@app.on_callback_query(filters.regex("^help_index$"))
async def help_index_cb(client, cb: CallbackQuery):
    """Help index"""
    try:
        await cb.answer()
        cb.message.from_user = cb.from_user
        ...
        logger.error(f"Error in help_index_cb: {e}")
        await cb.answer("Error!", show_alert=True)
