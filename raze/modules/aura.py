...
logger = logging.getLogger("RazeChan.Aura")
...
async def get_aura(user_id: int, chat_id: int) -> dict:
    ...
    try:
        ...
        logger.error(f"Error in get_aura: {e}")
        return {"points": 0, "given_today": 0, "stolen_today": 0}
...
async def update_aura(user_id: int, chat_id: int, delta: int):
    ...
    try:
        ...
        logger.error(f"Error updating aura: {e}")

# ── /aura command ─────────────────────────────────────────────────────────
@app.on_message(filters.command("aura"))
async def aura_cmd(client, msg: Message):
    """Show aura stats"""
    try:
        user = msg.from_user
...
async def give_aura(client, msg: Message):
    ...
    try:
        ...
        logger.error(f"Error in give_aura: {e}")
        await msg.reply("oops error! 😅")

# ── /steal command ─────────────────────────────────────────────────────────
@app.on_message(filters.command("steal") & filters.reply)
async def steal_aura(client, msg: Message):
    """Steal aura from another user"""
    try:
        thief = msg.from_user
        ...
        logger.error(f"Error in steal_aura: {e}")
        await msg.reply("oops error! 😅")

# ── /leaderboard ─────────────────────────────────────────────────────────
@app.on_message(filters.command("leaderboard"))
async def leaderboard(client, msg: Message):
    """Show aura leaderboard"""
    try:
...
async def aura_lb_cb(client, cb: CallbackQuery):
    """Show aura leaderboard from callback"""
    try:
        await cb.answer()
        top = await aura_col.find(
        ...
        logger.error(f"Error in aura_lb_cb: {e}")
